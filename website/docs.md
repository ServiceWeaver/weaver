...
...
```
the output generated by the code sample bellow 
```go
type Adder interface {
    Add(context.Context, int, int) (int, error)
}
...
func (a *adder) Init(ctx context.Context) {
    a.SetLogLevel(slog.LevelError)
}
```
will look like:
```console
...
| log_level | optional | Application log level. If not provided, service weaver will pick a default value(debug). |
