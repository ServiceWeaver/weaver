...
func RunManager(ctx context.Context, dep *protos.Deployment, locations []string, logDir string, tmpDirs map[string]string) (func() error, error) {
	// Create log saver.
...
func (m *manager) startBabysitter(loc string, info *BabysitterInfo) error {
	...
	binaryPath := filepath.Join(m.tmpDirs[loc], "weaver")
